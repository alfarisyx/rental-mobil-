        ...
        <div class="p-4  mt-14 bg-white">
            ...
            <div class="grid grid-cols-3 gap-4 mb-4 bg-white">
                ...
                <ul class="menu menu-horizontal rounded-lg bg-base-200 shadow-lg">
                    <li><a href="approve.php" class="btn btn-ghost btn-md bg-gray-700 m-2">Approve</a></li>
...
$sql = "SELECT tbl_transaksi.*, tbl_mobil.foto FROM tbl_transaksi JOIN tbl_mobil WHERE tbl_transaksi.nopol = tbl_mobil.nopol AND tbl_transaksi.status = 'booking'";
        ...
        ?>
                        ...
                        <tr
                            ...
                            <div class="flex space-x-2">
                                     ...
                                     <?php if ($_SESSION['level']== 'petugas' && $row['status'] == 'booking'): ?>
                                     <?php $button = '<a href="#" onclick="document.getElementById(\'aprove_modal_' . $row['id_transaksi'] . '\').showModal()" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">approve</a>'; ?>
                                     <?php elseif ( ($_SESSION['level']== 'petugas' && $row['status'] == 'approve')): ?>
                                     <?php $button = '<span class="font-medium text-red-600 dark:text-red-500"> beralih ke form ambil </span>'; ?>
                                     <?php else : ?>
                                     <?php $button = '<span class="font-medium text-red-600 dark:text-red-500"> sedang di proses </span>'; ?>
                                     <?php endif; ?>
                                     <?= $button ?>
...
while ($row = $result->fetch_assoc()) {
?>
    <dialog id="aprove_modal_<?php echo $row['id_transaksi']; ?>" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box">
            <h3 class="text-lg font-bold">ACC Bookingan</h3>
            <p class="py-4 m-4">
                <?php echo $row['nopol']; ?>?</p>
            <div class="modal-action w-full">
                <form method="POST" class="justify-items-center w-full" action="" enctype="multipart/form-data">
                    ...
                    <div class="modal-action">
                        ...
                        <button class="btn mt-4" type="button"
                            onclick="document.getElementById('aprove_modal_<?php echo $row['id_transaksi']; ?>').close()">Batal</button>
                    </div>
                </form>
            </div>
        </div>
...
if ($_SERVER['REQUEST_METHOD']== 'POST'){
    ...
    $sql = "UPDATE tbl_transaksi SET status = 'approve' where id_transaksi = ?";
    $stmt = $koneksi->prepare($sql);
